...
- **MCP Server** (`@bacchus/mcp`): Stdio-based MCP server exposing the full VINE mutations API (~12 tools) for AI tool-use. Read-only tools (`vine_validate`, `vine_show`, `vine_list`, `vine_get_task`, `vine_get_descendants`, `vine_search`) and mutation tools (`vine_add_task`, `vine_remove_task`, `vine_set_status`, `vine_update_task`, `vine_add_dependency`, `vine_remove_dependency`). Standalone package using `@modelcontextprotocol/sdk`, reusing `@bacchus/core` pure functions with isolated file I/O.

...
- **VINE v1.2.0 — Sprite System** (v1.2.0): Header annotations (`@key(value)` syntax), `@sprite(uri)` annotation for custom per-node sprites, animated SVG `<symbol>` sprite system replacing procedural gradient stack, feColorMatrix per-status tinting, sprite registry with Vite raw import, reference-node legend entry, sidebar sprite URI display.
- **MCP Server** (`@bacchus/mcp`): Stdio-based MCP server exposing the full VINE API (17 tools) for AI tool-use. Read-only tools (`vine_validate`, `vine_show`, `vine_list`, `vine_get_task`, `vine_get_descendants`, `vine_search`), mutation tools (`vine_add_task`, `vine_remove_task`, `vine_set_status`, `vine_update_task`, `vine_add_dependency`, `vine_remove_dependency`), ref tools (`vine_add_ref`, `vine_expand_ref`, `vine_update_ref_uri`, `vine_get_refs`), and the execution tool (`vine_next_tasks`). Standalone package using `@modelcontextprotocol/sdk`, reusing `@bacchus/core` pure functions with isolated file I/O.
- **Execution Tool — `vine_next_tasks`** (`@bacchus/mcp`): Advisory tool that analyses graph state and returns the execution frontier — tasks ready to start, reviewing tasks safe to complete, and ref nodes needing expansion. Pure `getActionableTasks` function in `@bacchus/core` with MCP wrapper. Enables AI agents to drive `.vine` files as executable task graphs with parallel sub-agent dispatch. Documented in `AGENTS.md`.
- **VS Code Extension** (`@bacchus/vscode`): Extension that bundles the MCP server for automatic tool discovery by VS Code Copilot. Activates on `.vine` file open, registers a `bacchus.showGraph` command, watches `.vine` files for changes, and hosts a webview visualization panel. MCP tools are available to AI assistants without separate installation.

## Planned

Features under active development:

- **Live Graph Webview**: Stripped-down Svelte/D3 graph visualization embedded in the VS Code extension webview. Read-only view showing nodes (status-colored circles with labels), directed edges (Bézier paths with arrowheads), and pan/zoom. Receives live updates from the extension host via `postMessage` on file change. Adapts to VS Code light/dark themes via `--vscode-*` CSS variables.

## Exploring

Ideas under consideration for future development:

- **VS Code Diagnostics**: Parse/validation error squiggles in `.vine` files, mapping `VineParseError.line` and `VineValidationError` to VS Code diagnostic ranges
- **VS Code Task Tree View**: Sidebar tree view for navigating tasks in `.vine` files, with status icons and click-to-reveal

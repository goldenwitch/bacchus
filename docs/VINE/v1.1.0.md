# VINE Format Specification — Version 1.1.0
...
Version 1.1.0 is a backward-compatible extension of v1.0.0. A v1.1.0 parser **must** also accept v1.0.0 files. The additions are:

| Feature                   | Summary                                                                                     |
| ------------------------- | ------------------------------------------------------------------------------------------- |
| **Reference nodes**       | A new node type (`ref [id] Name (URI)`) that acts as a proxy for an external `.vine` graph. |
| **`prefix` metadata key** | Controls ID namespacing when this graph is expanded into a parent graph.                    |
| **Expansion semantics**   | An informational algorithm for inlining a referenced graph into a parent.                   |

A v1.0.0 file contains no reference nodes and no `prefix` metadata — the parser produces the same result regardless of which version it was written for.
...
[root] Root Task (started)    ← first block = root (task)
...
===                           ← block separator (custom delimiter)
ref [sub] Sub-Project (./sub-project.vine)
                              ← reference (URI in parens)
===
...
where `<version>` is a [SemVer](https://semver.org/) string. For this specification: `vine 1.1.0`.
...
Both block types share a single ID namespace.
...
```
^\[([a-zA-Z0-9-]+(?:/[a-zA-Z0-9-]+)*)\]\s+(.+?)\s+\((complete|started|reviewing|planning|notstarted|blocked)\)$
```
...
| `Short Name` | Free text between `]` and `(`. Leading/trailing whitespace trimmed.                                                                                                                                                                  |
| `URI`        | Enclosed in `( )` at end of line. Non-whitespace printable ASCII. Follows [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986). Relative URIs resolved against the `.vine` file's location. Must point to a valid `.vine` file. |
...
task-header    = "[" task-id "]" 1*WSP short-name 1*WSP "(" status ")"
...
ref-header     = %s"ref" 1*WSP "[" task-id "]" 1*WSP short-name 1*WSP "(" uri ")"
...
5. For each raw node block:
   a. The first non-empty line is the header.
      Try matching the reference header regex: ref [id] Name (URI)
      If it matches → parse as reference block (step 5r).
      Otherwise, try the task header regex: [id] Name (status)
      If it matches → parse as task block (step 5t).
      Otherwise → reject (invalid header).

   5t. Task block:
       Extract id, short name, and status.
   ...
   5r. Reference block:
       Extract id, short name, and URI from the header.
       Classify each subsequent line by prefix (priority order):
         - Starts with "-> "       → dependency
         - Starts with "> "        → decision
...
6. **Reference block body line order**:
   - Description lines (preserving internal newlines)
   - Dependencies (`-> ...`), sorted alphabetically by target id
   - Decisions (`> ...`), in original order
7. **Trailing newline** after the final block (no trailing delimiter)
...
3. The reference node becomes a concrete task with `ship`'s status (`started`), description, and dependencies (`-> ds/components`, `-> ds/docs`). The ref node had no additional deps, so no merging needed.
...
-> design-system
---
[design-system] Ship Design System v1 (started)
Release the first version of the shared design system.
-> ds/components
-> ds/docs
---
